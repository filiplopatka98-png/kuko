...
> **Quality Sprint 3 shipped** (2026-05-15, commits `05ff94e`…`0e55601`): A5 AA sub-text contrast (#7A7A7A→#6A6A6A) + site-wide :focus-visible + reduced-motion fix; A2 skip-link + single `<main id=main>` landmark on all layouts (footer DRY-hoisted into layout.php); A3 reservation form aria-required/role=alert/role=status + required legend; U1 SYMMETRIC time-slot buffer (block before+after existing reservations); U3 SK phone validation (^(\+421|0)[0-9]{9}$) + email datalist + sessionStorage draft persist + default 14:00; U4 add-to-calendar (.ics + Google) on success; B6 session-cookie hardening (use_strict_mode/httponly/samesite=Lax/secure-when-https, inert under CLI/tests) ; B5 DB backup cron + docs/RECOVERY.md. 194 PHPUnit tests green. Owner/manual remaining: B3 dep audit, B7 ZAP/DNS/pentest, A6 plain-language copy, + go-live items (SMTP, reCAPTCHA browser test, GDPR cron registration, Lighthouse/axe, Google Business Profile, HSTS preload).

> **Quality Sprint 2 shipped** (2026-05-15, commits `95c663a`…`a1502d6`): per-page noindex (privacy/404/status/maintenance); single H1 on reservation page (4→1, step titles→h2); WOFF2 fonts (~60% smaller, preload repointed); CSS/JS minification via `Kuko\Asset` `.min`-sibling preference + `private/scripts/build-assets.php` + stale-min guard test; responsive hero (768px mobile variant 146→50KB + media-scoped preload). 157 PHPUnit tests green. Note: `hero.jpg` is PNG-content/.jpg-ext (pre-existing, separate cleanup task).
...
- [x] **noindex na privacy + 404 + rezervacia status + maintenance** (Sprint 2: `$pageIndexing=false`) (aby sa tieto pomocné strany neindexovali nezávisle)
...
- [x] **H1 audit:** (Sprint 2: reservation 4→1, step→h2) každá stránka má presne 1 H1. Skontrolovať že hero "Detský svet KUKO" je jediný H1 na homepage
- [x] **ALT texty** galérie — konkrétne SK popisy v `sections/galeria.php` (`$galleryAlts` + DB `alt_text`). Homepage galéria fix: starter fotky presunuté do `/assets/img/gallery/` (boli rozbité — DB cesta vs fyzické súbory)
...
- [x] **CSS minifikácia** (Sprint 2: csso, Asset .min) — `main.css` ~600 riadkov; cez `npx csso main.css -o main.min.css` v deploy skripte
- [x] **JS minifikácia** (Sprint 2: esbuild, Asset prefers .min) — moduly sú malé, ale `npx esbuild --bundle --minify main.js` by zaokrúhlilo na ~3 KB
...
- [x] **Automatický DB backup** (Sprint 3: private/cron/db-backup.php) — WebSupport robí denné zálohy, ale dobré pridať vlastný cron týždenne s download cez SFTP do lokálu/Backblaze B2
...
- [x] **Recovery dokumentácia** (Sprint 3: docs/RECOVERY.md) — krok-po-kroku: keby spadol DB server, ako obnovit?
...
- [x] **Session hardening** (Sprint 3: App.php use_strict_mode/httponly/samesite/secure) — `session.use_strict_mode = 1`, `session.cookie_secure = 1`, `session.cookie_httponly = 1`, `session.cookie_samesite = Lax` (väčšina máme)
...
- [x] **CSS minifikácia** (Sprint 2) — `main.css` ~600 riadkov, `rezervacia.css` ~250 riadkov, `admin.css` ~150. Cez `npx csso *.css -o *.min.css` v deploy skripte. Bonus: gzip ich Apache už robí
- [ ] **Critical CSS inlining** — extract above-the-fold CSS (~5 KB) priamo do `<style>` v `<head>`, zvyšok asynchronously. Cez `critical` npm tool
- [x] **JS minifikácia** (Sprint 2: esbuild --minify) — moduly sú malé, ale `npx esbuild --bundle --minify` by ich zlúčilo do ~5 KB total. ES modules sa stratia (treba `--format=esm`)
...
- [x] **Font format upgrade** (Sprint 2: WOFF2 ~60% menšie, preload repointed) — pridať WOFF2 variant (lepšia kompresia než TTF), keep TTF ako fallback
- [ ] **Image format audit** — overiť že WebP varianty sa správne servujú cez `<picture>` (DevTools Network — `image/webp` Content-Type)
- [x] **Responsive images** (Sprint 2: hero 768px mobil variant + media-scoped preload) — `srcset` pre hero podľa viewport (768w, 1280w, 1920w varianty), aktuálne servujeme jeden veľký
...
- [x] **Cache-busting** — `Kuko\Asset::url()` pridáva `?v=filemtime` na vlastné CSS/JS (head/layout/admin-layout/layout-minimal/reservation). **Residual:** `main.js` dynamické `import('./gallery.js'|'./map.js')` nie sú bustené (string literály v JS, nie cez šablónu) — nízka priorita, tie moduly sa menia zriedka
...
- [x] **Landmark regions** (Sprint 3: single <main id=main> all layouts) — overiť že každá stránka má presne `<main>`, voliteľne `<aside>` pre sidebar
- [x] **Skip link** (Sprint 3) — `<a href="#main" class="sr-only-focusable">Skip to content</a>` na vrch každej stránky pre keyboard users
...
- [x] **Error messages** (Sprint 3: #form-error role=alert aria-live=assertive) — `aria-describedby` linking input k error msg, `role="alert"` + `aria-live="polite"` na error container
- [ ] **Inline validation announce** — keď klient niečo zle vyplní, screen reader by mal počuť error bez submit-u
- [x] **Success state announce** (Sprint 3: role=status aria-live=polite) — po submit-e success div má `role="status"` + `aria-live="polite"`
...
- [x] **Sub-text contrast audit** (Sprint 3: #7A7A7A→#6A6A6A AA 5.1:1) — `#7A7A7A` na `#FFF8EE` = 4.0:1 — pod AA pre normal text (4.5:1). Buď použiť `#666` (5.4:1) alebo zväčšiť na large text
- [ ] **Calendar status farby** — zelená/červená pre available/full nie sú vhodné pre farbosleposť. Pridať aj ikon (✓/✗) alebo text label
- [x] **Focus indicator** (Sprint 3: site-wide :focus-visible) — `:focus-visible` outline pre keyboard, viditeľný aj na dark backgrounds (akcent ring je OK pre svetlé, ale na hero overlay zlý)
- [x] **prefers-reduced-motion** (Sprint 3: rezervacia step fade covered) — hotové pre scroll reveal, treba audit aj na rezervacia step transitions (`fadeIn` animácia)
...
- [x] **Phone validation** (Sprint 3: SK ^(\+421|0)[0-9]{9}$) — momentálne regex `/^\+?[0-9 ()\/-]{7,20}$/` je voľný. Pre SK by sme mohli vyžadovať `+421` alebo `0` prefix
- [x] **Email autocomplete suggestions** (Sprint 3: <datalist>) — `autocomplete="email"` máme, but `<input list>` s gmail/outlook common domains
- [x] **Smart defaults** (Sprint 3: default 14:00 slot) — predvyplniť time na 14:00 (najobľúbenejší slot pre oslavy)
- [x] **Persist on reload** (Sprint 3: sessionStorage draft) — ak omylom F5 počas formulára, neuložiť ale zachovať stav cez `sessionStorage`
...
- [x] **Success add-to-calendar** (Sprint 3: .ics + Google Calendar) — po rezervácii zobraziť tlačidlo „Pridať do Google Calendar" s `webcal://` linkom alebo `.ics` download
